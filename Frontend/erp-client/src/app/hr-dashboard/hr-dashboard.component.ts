          ...
          <div class="p-6 border-b border-slate-100 flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4">
            <div>
              ...
              <p class="text-sm text-slate-500 mt-1">
                @if (searchTerm()) {
                  Tìm thấy <strong class="text-indigo-600">{{ totalCount() }}</strong> nhân viên
                  với từ khóa "<span class="font-semibold text-slate-700">{{ searchTerm() }}</span>"
                } @else {
                  Tổng cộng <strong class="text-indigo-600">{{ totalCount() }}</strong> nhân viên
                }
              </p>
            </div>
            ...
            @if (totalPages() > 1) {
              <div class="px-6 py-4 border-t border-slate-100 flex items-center justify-between">
                <div class="text-sm text-slate-500">
                  Trang <span class="font-medium text-slate-800">{{ pageNumber() }}</span> / <span class="font-medium text-slate-800">{{ totalPages() }}</span>
                </div>
                <div class="flex gap-2">
                  <button 
                    (click)="changePage(pageNumber() - 1)" 
                    [disabled]="pageNumber() === 1"
                    class="px-3 py-1 border border-slate-200 rounded-lg text-sm font-medium hover:bg-slate-50 disabled:opacity-50 disabled:cursor-not-allowed transition-colors">
                    Trước
                  </button>
                  <button 
                    (click)="changePage(pageNumber() + 1)" 
